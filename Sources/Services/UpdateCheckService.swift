...
/// The manifest MacHub polls to learn a newer version exists — served by
/// gogenops.com's shared `/api/update-check` endpoint (see mac-groom's
/// `UpdateCheckService.swift`, the reference this file mirrors), which
/// fronts a small, hand-edited static JSON manifest at
/// gogenops.com/mac-apps/machub/updates.json —
/// `{"version": "1.4", "notes": "...", "url": "https://..."}`. Going
/// through the endpoint rather than fetching that file directly gives
/// gogenops.com a record of what versions are actually still installed and
/// running; adding `v`/`os`/`app` query params changes nothing about what
/// comes back.
...
/// MITM on a plain HTTP mirror, a DNS-level attacker — could point MacHub
/// at an arbitrary binary) this app doesn't have yet. Silent
...
enum UpdateCheckService {
    static func checkForUpdate() async -> UpdateManifest? {
        ...
        components.queryItems = [
            URLQueryItem(name: "app", value: "machub"),
